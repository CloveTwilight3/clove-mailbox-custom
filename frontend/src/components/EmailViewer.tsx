...
import { X, Star, Reply, Forward, Trash2, Archive, ChevronLeft, ChevronRight, Mail, ExternalLink } from 'lucide-react'
...
interface EmailViewerProps {
  ...
  hasNext?: boolean
  hasPrevious?: boolean
  isStandalone?: boolean // New prop for standalone email viewer
}
...
const EmailViewer = ({ 
  ...
  // Get email ID from props or URL params
  const emailId = propEmailId || parseInt(searchParams.get('emailId') || '0')
  
  const { data: email, isLoading, error } = useEmail(emailId)
  const updateEmailMutation = useUpdateEmail()
  const [showRawHeaders, setShowRawHeaders] = useState(false)

  // Handle browser back/forward for standalone viewer
  useEffect(() => {
    if (isStandalone && emailId) {
      // Mark as read when opened in standalone mode
      updateEmailMutation.mutate({
        emailId,
        updates: { is_read: true }
      })
    }
  }, [emailId, isStandalone])
  ...
  if (isLoading) {
    return (
      <div className={`${isStandalone ? 'min-h-screen' : 'fixed inset-0'} bg-black bg-opacity-50 flex items-center justify-center z-50`}>
  ...
  const EmailContent = () => (
    <div className={`${isStandalone ? 'min-h-screen' : ''} bg-white rounded-lg shadow-xl ${isStandalone ? '' : 'max-w-4xl w-full max-h-[90vh]'} flex flex-col`}>
      {/* Header */}
